...
=>
(do-for-all-facts ((?i item)) TRUE
(assert (test-item (name ?i:name)(quantity ?i:quantity)(price ?i:price)))
))

;;;(if (= (integer (fact-slot-value 1 quantity)) 6)
;;;                then (bind ?*price* (+ ?*price* 0))
;;;                else (bind ?*price* (- ?*price* 0))
;;;                )
)
